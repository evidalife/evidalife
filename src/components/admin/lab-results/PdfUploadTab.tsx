...
function ConfidenceBadge({ c }: { c: number }) {
  ...
  const cls = c >= 0.9 ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20'
    : c >= 0.7 ? 'bg-amber-50 text-amber-700 ring-amber-600/20'
    : 'bg-red-50 text-red-700 ring-red-600/20';
  const dot = c >= 0.9 ? '🟢' : c >= 0.7 ? '🟡' : '🔴';
  return <Badge className={cls}>{dot} {pct}%</Badge>;
}
...
export default function PdfUploadTab() {
  ...
  const loadUploads = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('id, file_name, extraction_status, results_created, created_at, extracted_data, profiles:uploaded_by(first_name, last_name)')
  ...
  const handleUploadAndExtract = async () => {
    ...
    const { error: storageError } = await supabase.storage.from('lab-pdfs').upload(path, file, { upsert: false });
    ...
    const { data: urlData } = await supabase.storage.from('lab-pdfs').createSignedUrl(path, 3600);
    ...
    const res = await fetch('/api/admin/parse-lab-results', {
      ...
      body: JSON.stringify({ fileUrl, uploadId: uploadRecord.id }),
    });
  ...
  const handleSaveExtracted = async () => {
    ...
    const results = toSave.map((r) => {
      ...
      return {
        ...
        biomarkerName: r.matched_name || r.biomarker_name_en,
        refRangeLow: def?.ref_range_low ?? r.reference_range_low,
        refRangeHigh: def?.ref_range_high ?? r.reference_range_high,
                ...
                <div className="space-y-2">
                  <div className="h-2 rounded-full bg-[#0e393d]/8 overflow-hidden">
                    <div
                      ...
                      style={{ width: uploading ? `${uploadProgress}%` : '90%' }}
                    />
                  </div>
                  <p className="text-xs text-[#1c2a2b]/50 text-center">
                    {uploading ? 'Uploading…' : 'Extracting results with AI… (this may take 20-30 seconds)'}
                  </p>
                </div>
              )}
              ...
              <table className="w-full text-sm min-w-[600px]">
                <thead>
                  <tr className="border-b border-[#0e393d]/8 bg-[#0e393d]/3">
                    {['File', 'Uploaded by', 'Date', 'Status', 'Results'].map((h) => (
                ...
                <tbody className="divide-y divide-[#0e393d]/6">
                  {loadingUploads ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center"><div className="inline-flex justify-center"><Spinner /></div></td></tr>
                  ) : uploads.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-[#1c2a2b]/40">No uploads yet.</td></tr>
                  ) : uploads.map((u) => (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setExtracted([]); setFile(null); }} className="text-xs text-[#1c2a2b]/50 hover:text-[#1c2a2b]">← Back</button>
            </div>
          </div>
            ...
            <table className="w-full text-xs min-w-[860px]">
              <thead>
                <tr className="border-b border-[#0e393d]/8 bg-[#0e393d]/3">
                  ...
                  <th className="px-3 py-2.5 text-left font-medium text-[#0e393d]/60 uppercase tracking-wider">Unit</th>
                ...
                {extracted.map((row, idx) => {
                  const flag = row.db_biomarker
                    ? computeStatusFlag(row.value, {
                        ref_range_low: row.db_biomarker.ref_range_low ?? row.reference_range_low,
                        ref_range_high: row.db_biomarker.ref_range_high ?? row.reference_range_high,
                  ...
                  return (
                    <tr key={idx} className={`${row.include ? '' : 'opacity-40'} hover:bg-[#fafaf8] transition-colors`}>
                      ...
                      <td className="px-3 py-2.5 text-[#1c2a2b]">{row.biomarker_name}</td>
                      <td className="px-3 py-2.5">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            updateRow(idx, 'confidence', e.target.value ? 0.85 : 0);
                          }}
                          className="rounded border border-[#0e393d]/15 bg-white px-2 py-1 text-xs text-[#1c2a2b] focus:outline-none focus:ring-1 focus:ring-[#0e393d]/20 w-48"
                        >
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={() => { setExtracted([]); setFile(null); }}
